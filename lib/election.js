...
const dgram                    = require('dgram')
    , debug                    = require('debug')('zmqbus:node:elector')
    , {EventEmitter}           = require('events')
    , {lpad, random, isNumber} = require('./common')
    ;
...
class Elector extends EventEmitter{
  constructor(options = {}, metadata = {}){
    super();
    this.options              = Object.assign( {}, base_options, options );

    // check for numbers
    for(let key in this.options){
      if( /port/.test( key ) && !isNumber( this.options[key] ) ){
        throw new Error(`${key} must be a number - ${this.options[key]}`);
      }
    }

    this.metadata             = Object.assign( {}, metadata );
    this.sock                 = dgram.createSocket({type:'udp4', reuseAddr: true});
    this.id                   = lpad(99 - this.options.election_priority,2) + lpad( random(), 8)
  ...
  broadcast(advert){
    ...
    let msg = Buffer.from(JSON.stringify(advert));
  ...
  on_elect_msg(msg) {
    if( msg[0] == 'start' ){
      ...
      if( msg[1].id == this.elect_last.id ){
        ...
        debug(`node ${this.id} is the leader`)
